...
from data_loader_multi_timestep import load_test_data
from data_loader_multi_timestep import load_train_data
...
lead = 1
testN=1000
psi_test_input_Tr_torch, psi_test_label_Tr_torch, _, psi_test_label_Tr  = load_test_data(FF,lead, testN)
###############################################################################
...
net = CNN()
net.load_state_dict(torch.load('BNN_UNET_lead'+str(lead)+'.pt'))
net.cuda()
net.eval()
...
for epoch in range(0, num_epochs):  # loop over the dataset multiple times
    ...
    for loop in fileList_train:
     ...
     psi_input_Tr_torch, psi_label_Tr_torch, psi_label_next_Tr_torch = load_train_data(loop, lead, trainN)

     for step in range(0,trainN,batch_size):
        # get the inputs; data is a list of [inputs, labels]
        indices = np.random.permutation(np.arange(start=step, stop=step+batch_size))
        input_batch, label_batch, label_next_batch = psi_input_Tr_torch[indices,:,:,:], psi_label_Tr_torch[indices,:,:,:],psi_label_next_Tr_torch[indices,:,:,:]
        ...
        output2,_,_,_,_,_,_ = net(output.cuda())

        loss1 = loss_fn(output, label_batch.cuda())
        loss2 = loss_fn(output2, label_next_batch.cuda())
        loss = (loss1 + loss2)/2
        loss.backward()
        optimizer.step()
...
torch.save(net.state_dict(), './BNN_UNET_multitime_step_lead'+str(lead)+'.pt')

print('BNN Model Saved')

if (FLAGS_ACTIVATIONS_DUMP ==1):
 savenc_for_activations(Act_encoder, Act_decoder1, Act_decoder2,output_training,2,num_epochs,4,num_samples,64,128,192,192,96,path_static_activations+'BNN_UNET_multitime_step_Activations_Dry5_'+str(trainN)+'sample_size'+str(num_samples)+'_dt'+str(lead)+'.nc')
...
if (FLAGS_WEIGHTS_DUMP ==1):
 ...
 hdf5storage.write(matfiledata, '.', path_weights+'BNN_UNET_multitime_step_Weights_Dry5_'+str(trainN)+'sample_size'+str(num_samples)+'_dt'+str(lead)+'.mat', matlab_compatible=True)
...
savenc(autoreg_pred, lon, lat, 'predicted_multi_time_lead'+str(lead)+'.nc')
savenc(psi_test_label_Tr, lon, lat, 'truth_multi_time_lead'+str(lead)+'.nc')
